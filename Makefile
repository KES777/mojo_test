...
export APP      ?=  ${APP_ROOT}/bin/my_app
...
prod:
	@MOJO_MODE=staging MOJO_LOG_LEVEL=debug hypnotoad ${APP}
...
docker-dbdump:
	docker exec ${DOCKER_CONTAINER} pg_dump -U postgres ${DB_NAME} | gzip -f > \
		${APP_ROOT}/db/${DB_NAME}_$$(date "+%Y-%m-%d_%H-%M-%S").sql.gz
